DELIMITER $$
CREATE PROCEDURE `Get_New_SETS_Registrations`()
BEGIN
...
as
(
SELECT IFNULL(facility_mflcode,'Total') AS facility_mflcode, COUNT(system_id) AS total_registered from tbl_person where date_created=DATE(NOW()) group by facility_mflcode WITH ROLLUP 
);
